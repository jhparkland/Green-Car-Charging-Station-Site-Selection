        ...
        if affect == True:
            ...
            annotations.append(
                dict(x=(max+min)/2, y=norm.pdf(mean, loc=mean, scale=std), showarrow=False, text=round(1 - pro, 3),
                     font=dict(size=15, color=blue), xshift=30, yshift=40, bordercolor=blue, borderwidth=2))
            pro = 1 - pro
        fig.update_layout(annotations=annotations, title_font_size=22, margin_l=10, margin_r=10, margin_t=10, margin_b=10, font_family='NanumSquare',
                          legend_orientation="h", legend_x=0.4, legend_y=1.35)
...
#
#     def __init__(self):
#         self.parking_file_path = '부산 주차장 현황.csv'
