...
# 해당 컬럼 float 타입으로 변경
air_quality.advanced_replace(df_ozone, df_ozone.iloc[:,2:].columns.tolist(), '-', r'[^0-9.0-9]')
df_ozone['2021.07'] = df_ozone['2021.07'].astype(float)


air_quality.show_norm(df_ozone.iloc[:,2].mean(), df_ozone.iloc[:,2].std(), df_ozone.iloc[:,2].min(), df_ozone.iloc[:,2].max())
### 부산광역시 오존의 누적확률 구하기


busan_oz = df_ozone[df_ozone['구분(2)']=='부산광역시'].loc[2,'2021.07']
fig_ozone = air_quality.cal_norm(df_ozone.iloc[:,2].mean(),df_ozone.iloc[:,2].std(),df_ozone.iloc[:,2].min(),df_ozone.iloc[:,2].max(),busan_oz)
...
fig_ozone.update_layout({
    'paper_bgcolor': '#E9EEF6',
}, margin_l=10, margin_r=10, legend_y=1.5, legend_x=0.15, title_text='대기질',title_y=0.1,title_x=0.5)
...
chart = html.Div(
    dbc.Row([
        ...
        ], xs=12, sm=12, md=12, lg=4, xl=4, className="pie_chart"),
        ...
        ], xs=12, sm=12, md=12, lg=2, xl=2, className="chart_bar_1"),
